...
var scroll = {
	oScroll: function() {
		...
		var j = 0;
		var gdImg = document.getElementById("gd_goods");
...
//				console.log(gdImg.style.display)
			} else {
...
var goodsParameter = new Vue({
	...
	created: function() {
		...
		axios.get(
			...
			var space = document.getElementsByClassName("gd-goods-youhui")[0];
		...
		testclick: function(e) {
			...
			for(i in goodsParameter.attr) {
				...
				if(goodsParameter.attr[i].spec_2 == goodsParameter.text) {
					weight.push({
						'name': goodsParameter.attr[i].spec_3
					}); //获取商品大小
				}
			}
			console.log(weight);
